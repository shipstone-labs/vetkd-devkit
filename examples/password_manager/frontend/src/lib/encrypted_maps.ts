...
export async function createEncryptedMaps(agentOptions?: HttpAgentOptions): Promise<EncryptedMaps> {
    const CANISTER_ID_ENCRYPTED_MAPS_EXAMPLE = process.env.CANISTER_ID_ENCRYPTED_MAPS_EXAMPLE;
    ...
    return new EncryptedMaps(new DefaultEncryptedMapsClient(agent, CANISTER_ID_ENCRYPTED_MAPS_EXAMPLE));
}